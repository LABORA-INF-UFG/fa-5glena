...
for _ in range(view_client):
    ...
    while True:
        ...
        print(f"\nReceived response {received_data['client_id']}:")
        ...
        acknowledgment = "Response received successfully."
...
if received_data_list:
    ...
    if total_weight > 0:
        ...
        csv_target_file = 'sim_results/fa/fa_results.csv'# Definindo o diretório base
	               
        # Guarantee that the directoy exists, if not create it
        os.makedirs(os.path.dirname(csv_target_file), exist_ok=True)
