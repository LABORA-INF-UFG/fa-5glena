...
bool ReadPositionsFromCSV(const std::string &filename, std::vector<Vector> &positions)
{
    ...
    if (!std::filesystem::exists(folderPath)) {
        if (!std::filesystem::create_directories(folderPath)) {
            ...
            exit(1);  // Exit if directory creation failed
        }
    }
    ...
    if (outFile.tellp() == 0) {  // Only write header if the file is new (empty)
...
double getTxPowerFromCsv(int ueid, const std::string& fileName) {
    ...
    while (std::getline(inFile, line)) {
        std::stringstream ss(line);
        std::string ueid_str, txpower_str;

        std::getline(ss, ueid_str, ',');  
        std::getline(ss, txpower_str);    

        int currentUeid = std::stoi(ueid_str);  
        double txpower = std::stod(txpower_str);  
    ...
    std::cerr << "Error: ueid " << ueid << " not found in the file." << std::endl;
    return -1.0;
}
...
double calculateSignalPower(double txPower, double distance, double frequency, double hBs, double hUt) {
    ...
    // Line of Sight path loss 32.4 + 20 * std::log10(distance/1m) + 20 * std::log10(frequency/1e9);
    double losPathLoss = 32.4 + 21 * std::log10(distance/1) + 20 * std::log10(frequency/1e9) + 4.0;   	
    // Non Line of Sight path loss: 22.4 + 35.3 * std::log10(distance/1m) + 21.3 * std::log10(frequency/1e9) - 0.3 * (hUt - 1.5);
...
double CalculateMinimumTxPowerForUE(double minTxPower, double maxTxPower, double frequency, double distance, 
    ...
    // Start from initial TxPower and begin iteration
    double currentTxPower = std::abs(minimumTxPowerForUE);
    double snr, ber, per, dr;
    double stepSize = 3.0;  // Start with a larger step size for faster convergence
    double feedbackGain = 3.0; // Increase feedback gain to adjust power more aggressively
...
void runSimulation(uint16_t gNbNum, uint16_t ueNumPergNb, double maxTxPower, double minTxPower, double maxDistance, double seed, int numRun, double frequency, int argc)
{
  ...
  double centralFrequencyBand1 = 6.8e9;
...
mobility.Install(ueContainer);
  ...
  Ptr<Node> selectedUe = ueContainer.Get (numRun); // Selecting UEs from the container.  
  
  std::cout << "\nUE " << selectedUe->GetId() << " Transmiting... " << std::endl;  
    ...
    for (uint32_t k = 0; k < ueContainer.GetN(); ++k) {
        if (k != j) {  // Skip the current UE itself
            ...
            if(interferingDistance < 100){
                //double interferingTxPower = maxTxPower;
...
double reqTxPowerW = pow(10, reqTxPowerDbm / 10)/1000;
  ...
  for (uint32_t j = 0; j < 1; ++j)  // Only for the selected UE
  {
  ...
  // Print per-flow statistics for the selected UE
  ...
  // Prepare data for CSV    
    uint32_t flowId = 1;  // Start flow IDs at 1
    // Assuming closestGnbResults holds the UE and closest gNB mapping
    ...
    // Simulation results collection, printing and storing)
